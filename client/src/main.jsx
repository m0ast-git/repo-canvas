...
function Canvas({ snapshot, setSnapshot, toast, unauthorized, theme, toggleTheme, architect, setArchitect }) {
  ...
  const projectedNodes = useMemo(() => {
    ...
    const output = snapshot.areas.map((area) => { const rect = layoutAreas.get(area.id); const id = `area:${area.id}`; const manual = manualPositions.current.get(id); return rect && { id, type: "area", className: "area-shell", dragHandle: ".map-drag-surface", position: manual || { x: rect.x, y: rect.y }, width: rect.width, height: rect.height, draggable: true, selectable: false, zIndex: 0, style: { width: rect.width, height: rect.height }, data: { area, title: area.ownerTitle || area.title, description: area.ownerNote || area.note || "Смысловая граница ответственности", color: rect.color, distant, activeCount: distant ? activity.areas.get(area.id) || 0 : 0, labelScale: 1, muted: selectedArea !== "all" && selectedArea !== area.id, edit: openEdit } }; }).filter(Boolean);
    for (const entity of snapshot.entities) { const rect = detailPositions.get(entity.id); if (!rect) continue; const id = `entity:${entity.id}`; const manual = manualPositions.current.get(id); output.push({ id, type: "entity", dragHandle: ".map-drag-surface", position: manual || { x: rect.x, y: rect.y }, width: rect.width, height: rect.height, draggable: true, selectable: true, zIndex: rect.group ? 2 + (rect.depth || 0) : 5 + (rect.depth || 0), style: { width: rect.width, height: rect.height }, data: { entity, label: entity.ownerLabel || entity.label, description: entity.ownerPurpose || entity.purpose || entity.path || "Подтверждённый элемент проекта", color: colors.get(entity.areaId), status: entity.status, group: rect.group, activeCount: activityTier === "entity" ? activity.entities.get(entity.id) || 0 : 0, muted: selectedArea !== "all" && selectedArea !== entity.areaId, edit: openEdit, select: setSelectedEntity } }); }
    for (const work of snapshot.work.filter((item) => ACTIVE_WORK.has(item.status))) { const rect = workPositions.get(work.id); if (!rect) continue; const id = `work:${work.id}`; const manual = manualPositions.current.get(id); const first = entityMap.get(work.targets?.[0]); output.push({ id, type: "work", position: manual || { x: rect.x, y: rect.y }, width: rect.width, height: rect.height, draggable: false, selectable: false, zIndex: 20, style: { width: rect.width, height: rect.height }, data: { work, visible: activityTier === "work", color: colors.get(first?.areaId), open: openWork } }); }
    return output;
  ...
  const architectMessage = architect?.status === "failed" ? architect.error : architect?.status === "done"
    ? `${architect.result?.areas || 0} областей · ${architect.result?.entities || 0} элементов · ${architect.result?.relations || 0} связей${architect.result?.repairs ? ` · исправлений: ${architect.result.repairs}` : ""}`
