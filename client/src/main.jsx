...
const nodeTypes = { area: AreaNode, entity: EntityNode, person: PersonNode, work: WorkNode };
...
function Canvas({ snapshot, setSnapshot, toast, unauthorized, theme, toggleTheme, architect, setArchitect }) {
  ...
  const projectedNodes = useMemo(() => {
    ...
    for (const entity of snapshot.entities) { const rect = detailPositions.get(entity.id); if (!rect) continue; const id = `entity:${entity.id}`; const manual = manualPositions.current.get(id); const person = entity.kind === "person"; const relatedAreas = connectedAreas.get(entity.id) || new Set(); const color = person ? colors.get([...relatedAreas][0]) || "#d88961" : colors.get(entity.areaId); output.push({ id, type: person ? "person" : "entity", dragHandle: ".map-drag-surface", position: manual || { x: rect.x, y: rect.y }, width: rect.width, height: rect.height, draggable: true, selectable: true, zIndex: person ? 12 : rect.group ? 2 + (rect.depth || 0) : 5 + (rect.depth || 0), style: { width: rect.width, height: rect.height }, data: { entity, label: entity.ownerLabel || entity.label, description: entity.ownerPurpose || entity.purpose || entity.path || (person ? "Внешний участник продукта" : "Подтверждённый элемент проекта"), color, status: entity.status, group: rect.group, activeCount: person ? 0 : activityTier === "entity" ? activity.entities.get(entity.id) || 0 : 0, muted: selectedArea !== "all" && (person ? !relatedAreas.has(selectedArea) : selectedArea !== entity.areaId), edit: openEdit, select: setSelectedEntity } }); }
    ...
    return output;
  }, [layout, snapshot, activityTier, layoutAreas, detailPositions, workPositions, activity, selectedArea, entityMap, connectedAreas, colors, openEdit, openWork]);
