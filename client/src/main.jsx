...
const ACTIVE_WORK = new Set(["active", "blocked", "planned"]);
const NODE_READABLE_ZOOM = .42;
const WORK_READABLE_ZOOM = .72;
const THEME_KEY = "repo-canvas.theme";
const nodeKinds = { capability: "ВОЗМОЖНОСТЬ", module: "МОДУЛЬ", service: "СЕРВИС", process: "ПРОЦЕСС", store: "ХРАНИЛИЩЕ", interface: "ИНТЕРФЕЙС", integration: "ИНТЕГРАЦИЯ", external: "ВНЕШНЯЯ СИСТЕМА", component: "КОМПОНЕНТ" };
...
const WorkNode = memo(({ data }) => <button className={`work-node ${data.visible ? "is-readable" : "is-concealed"} ${data.work.status} ${data.work.provisional ? "provisional" : ""}`} style={{ "--area-color": data.color }} type="button" title={data.work.session ? "Двойной клик: открыть рабочую сессию" : "Сессия не привязана"} onDoubleClick={(event) => { event.stopPropagation(); data.open(data.work); }}>
...
const nodeTypes = { area: AreaNode, entity: EntityNode, work: WorkNode };
...
function descendants(snapshot) {
  const children = new Map(); for (const entity of snapshot.entities) { if (!entity.parentId) continue; if (!children.has(entity.parentId)) children.set(entity.parentId, []); children.get(entity.parentId).push(entity.id); }
  const all = new Map();
  function visit(id) { if (all.has(id)) return all.get(id); const result = new Set(); for (const child of children.get(id) || []) { result.add(child); for (const nested of visit(child)) result.add(nested); } all.set(id, result); return result; }
  for (const entity of snapshot.entities) visit(entity.id); return all;
}
...
function activeRollup(snapshot) {
  ...
  for (const work of snapshot.work.filter((item) => item.status === "active")) {
    ...
    for (const target of work.targets || []) {
      ...
      while (current && !seen.has(current.id)) { seen.add(current.id); workEntities.add(current.id); workAreas.add(current.areaId); current = current.parentId ? byId.get(current.parentId) : null; }
    }
...
function Canvas({ snapshot, setSnapshot, toast, unauthorized, theme, toggleTheme }) {
  const { layout, routes: detailedRoutes, areaRoutes, routeDrag } = useLayout(snapshot); const wrapper = useRef(null); const size = useContainerSize(wrapper); const flow = useReactFlow();
  ...
  const projectedNodes = useMemo(() => {
    ...
    const output = snapshot.areas.map((area) => { const rect = layoutAreas.get(area.id); const id = `area:${area.id}`; const manual = manualPositions.current.get(id); return rect && { id, type: "area", className: "area-shell", dragHandle: ".map-drag-surface", position: manual || { x: rect.x, y: rect.y }, draggable: true, selectable: false, zIndex: 0, style: { width: rect.width, height: rect.height }, data: { area, title: area.ownerTitle || area.title, description: area.ownerNote || area.note || "Смысловая граница ответственности", color: rect.color, distant, activeCount: distant ? activity.areas.get(area.id) || 0 : 0, labelScale: 1, muted: selectedArea !== "all" && selectedArea !== area.id, edit: openEdit } }; }).filter(Boolean);
    for (const entity of snapshot.entities) { const rect = detailPositions.get(entity.id); if (!rect) continue; const id = `entity:${entity.id}`; const manual = manualPositions.current.get(id); output.push({ id, type: "entity", dragHandle: ".map-drag-surface", position: manual || { x: rect.x, y: rect.y }, draggable: true, selectable: true, zIndex: rect.group ? 2 + (rect.depth || 0) : 5 + (rect.depth || 0), style: { width: rect.width, height: rect.height }, data: { entity, label: entity.ownerLabel || entity.label, description: entity.ownerPurpose || entity.purpose || entity.path || "Подтверждённый элемент проекта", color: colors.get(entity.areaId), status: entity.status, group: rect.group, activeCount: activityTier === "entity" ? activity.entities.get(entity.id) || 0 : 0, muted: selectedArea !== "all" && selectedArea !== entity.areaId, edit: openEdit, select: setSelectedEntity } }); }
    for (const work of snapshot.work.filter((item) => ACTIVE_WORK.has(item.status))) { const rect = workPositions.get(work.id); if (!rect) continue; const id = `work:${work.id}`; const manual = manualPositions.current.get(id); const first = entityMap.get(work.targets?.[0]); output.push({ id, type: "work", position: manual || { x: rect.x, y: rect.y }, draggable: false, selectable: false, zIndex: 20, style: { width: rect.width, height: rect.height }, data: { work, visible: activityTier === "work", color: colors.get(first?.areaId), open: openWork } }); }
    return output;
  }, [layout, snapshot, activityTier, layoutAreas, detailPositions, workPositions, activity, selectedArea, entityMap, colors, openEdit, openWork]);
  ...
  const onNodeDragStart = useCallback((_, node) => { const sourceId = node.id.replace(/^[^:]+:/, ""); const affected = new Set([node.id]); const affectedEntities = new Set(); if (node.id.startsWith("area:")) for (const entity of snapshot.entities.filter((item) => item.areaId === sourceId)) { affected.add(`entity:${entity.id}`); affectedEntities.add(entity.id); } if (node.id.startsWith("entity:")) { affectedEntities.add(sourceId); for (const child of hierarchy.get(sourceId) || []) { affected.add(`entity:${child}`); affectedEntities.add(child); } } for (const work of snapshot.work.filter((item) => ACTIVE_WORK.has(item.status) && (item.targets || []).some((target) => affectedEntities.has(target)))) affected.add(`work:${work.id}`); const positions = new Map(nodes.filter((item) => affected.has(item.id)).map((item) => [item.id, { ...item.position }])); for (const [id, position] of positions) manualPositions.current.set(id, position); drag.current = { id: node.id, start: { ...node.position }, positions }; }, [snapshot.entities, snapshot.work, hierarchy, nodes]);
  const onNodeDrag = useCallback((_, node) => { const context = drag.current; if (!context || context.id !== node.id) return; const dx = node.position.x - context.start.x; const dy = node.position.y - context.start.y; const moves = [...context.positions].map(([id, initial]) => ({ id, x: initial.x + dx, y: initial.y + dy })); for (const move of moves) manualPositions.current.set(move.id, { x: move.x, y: move.y }); if (context.positions.size > 1) setNodes((current) => current.map((item) => { const initial = context.positions.get(item.id); return initial ? { ...item, position: { x: initial.x + dx, y: initial.y + dy } } : item; })); routeDrag(moves); }, [routeDrag]);
  const onNodeDragStop = useCallback(async (_, node) => {
    const context = drag.current; drag.current = null;
    if (!context || !node.id.match(/^(area|entity):/)) return;
    const kind = node.id.startsWith("area:") ? "area" : "entity";
    ...
    const before = [...context.positions].filter(([nodeId]) => /^(area|entity):/.test(nodeId)).map(([nodeId, initial]) => ({ kind: nodeId.startsWith("area:") ? "area" : "entity", id: nodeId.replace(/^[^:]+:/, ""), x: initial.x, y: initial.y }));
    const items = [...context.positions].filter(([nodeId]) => /^(area|entity):/.test(nodeId)).map(([nodeId, initial]) => ({ kind: nodeId.startsWith("area:") ? "area" : "entity", id: nodeId.replace(/^[^:]+:/, ""), x: initial.x + dx, y: initial.y + dy }));
    try {
      await enqueueMutation(() => persistLayout(items));
      remember({ type: "layout", label: kind === "area" ? "перемещение области" : "перемещение элемента", before, after: items });
      toast(kind === "area" ? "Область и всё содержимое перемещены" : "Элемент и его вложенная структура перемещены");
    } catch (error) {
      for (const [id, position] of context.positions) manualPositions.current.set(id, position);
      setNodes((current) => current.map((item) => context.positions.has(item.id) ? { ...item, position: context.positions.get(item.id) } : item));
      toast(error.message, true);
    }
  }, [enqueueMutation, persistLayout, remember, routeDrag, toast]);
  ...
  const runHistory = useCallback(async (direction) => {
    ...
    try {
      ...
      if (entry.type === "layout") {
        ...
        setNodes((current) => current.map((node) => { const value = values.find((item) => `${item.kind}:${item.id}` === node.id); return value ? { ...node, position: { x: value.x, y: value.y } } : node; }));
      }
  ...
  }, [enqueueMutation, persistLayout, refreshSnapshot, syncHistory, toast]);

  const fitAll = useCallback(() => { setSelectedArea("all"); requestAnimationFrame(() => flow.fitView({ padding: .1, duration: 450, maxZoom: 1.05 })); }, [flow]);
  const focusArea = useCallback((id) => { setSelectedArea(id); const ids = [`area:${id}`, ...snapshot.entities.filter((item) => item.areaId === id).map((item) => `entity:${item.id}`)]; requestAnimationFrame(() => flow.fitView({ nodes: nodes.filter((node) => ids.includes(node.id)), padding: .13, duration: 450, maxZoom: 1.1 })); }, [flow, snapshot.entities, nodes]);
  ...
  async function runRegenerate() { try { manualPositions.current.clear(); const response = await api("/api/architect/refresh", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ viewpoint }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error); setRegenerate(false); toast(result.started ? "Повторная генерация запущена" : "Architect уже работает"); } catch (error) { toast(error.message, true); } }
...
function App() {
  const [snapshot, setSnapshot] = useState(null); const [unauthorized, setUnauthorized] = useState(false); const [toastState, setToast] = useState(null); const [update, setUpdate] = useState(null); const [theme, setTheme] = useState(() => localStorage.getItem(THEME_KEY) || (matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light")); const toastTimer = useRef(null);
  ...
  useEffect(() => { let timer; async function check(force = false) { try { const response = await api(`/api/update/status?${force ? "refresh=1&" : ""}t=${Date.now()}`); if (response.ok) setUpdate(await response.json()); } catch {} timer = setTimeout(() => check(false), 60 * 60 * 1000); } check(true); return () => clearTimeout(timer); }, []);
  ...
  return <ReactFlowProvider><Canvas snapshot={snapshot} setSnapshot={setSnapshot} toast={toast} unauthorized={unauthorized} theme={theme} toggleTheme={() => setTheme((current) => current === "dark" ? "light" : "dark")} />{update && ["available", "failed", "applying"].includes(update.status) && <button className="update-button" onClick={applyUpdate} disabled={update.status === "applying"}><small>ДОСТУПНО ОБНОВЛЕНИЕ</small><strong>{update.status === "applying" ? "Устанавливается…" : `Update v${update.availableVersion || ""}`}</strong></button>}{toastState && <div className={`toast ${toastState.error ? "error" : ""}`}>{toastState.message}</div>}</ReactFlowProvider>;
}
